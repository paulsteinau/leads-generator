...
* **HERO TOP PADDING CAP (mandatory):** Hero top padding max `pt-24` (approx 6rem) at desktop. More than that means the hero content floats halfway down the viewport and reads as a layout bug, not as intentional space. If your hero needs more breathing room, increase font scale or asset size, not top padding.
...
* **Fake-precise numbers are flagged.** Numbers like `92%`, `4.1x`, `48k`, `5.8 mm`, `13.4 lb` either:
...
* Be aware of bundle size. Motion is not tiny.
...
* **8-10 (Advanced Choreography):** Complex scroll-triggered reveals, parallax. **NEVER use `window.addEventListener('scroll')`**.

### VISUAL_DENSITY (Level 1-10)
* **1-3 (Art Gallery):** Lots of white space. Huge section gaps (`py-32` to `py-48`).
* **4-7 (Daily App):** Standard web app spacing (`py-16` to `py-24`).
* **8-10 (Cockpit):** Tight paddings. `font-mono` for all numbers.
...
### 9.D Content & Data
* **NO generic names.** "John Doe" -> use realistic, locale-appropriate names.
* **NO generic avatars.**
...
**Hero & top-of-page**
* **NO version labels in the hero.** `V0.6`, `BETA`, `INVITE-ONLY PREVIEW` - banned as default eyebrows.
* **NO "Brand - No. 01"-style sub-eyebrows.**

**Section numbering & micro-labels**
* **NO section-number eyebrows.** `00 / INDEX`, `001 - Capabilities` - banned.
* **NO `01 / 4`-style pagination** on images or bento tiles.

**Separators & dots**
* **The middle-dot (·) is rationed.** Maximum 1 per line in metadata strips.
* **NO decorative colored status dots** on every list/nav/badge by default.

**Em-dashes & typography flourishes**
* **NO em-dash (`--`) as a design element OR anywhere else.** See Section 9.G.
* **NO `<br>`-broken-and-italicized headlines** as a default "design move."
* **NO vertical rotated text** as a default.
...
* **NO generic step labels.** "Stage 1 / Stage 2 / Stage 3" - banned.
...
* **Scroll cues are banned.** `Scroll`, `down scroll`, `Scroll to explore` - if the user hasn't scrolled, they know what scroll is.
...
* **Banned in en-dash form too (`-`) when used as a separator.** Date ranges (`2018-2026`) use a hyphen.
...
- [ ] **ZERO em-dashes (`--`) anywhere on the page.** Zero. (Section 9.G - non-negotiable.)
- [ ] **Page Theme Lock**: ONE theme for the whole page. No section flips.
...
- [ ] **Premium-consumer palette check**: palette is NOT the AI-default beige+brass family?
...
- [ ] **Zigzag Alternation Cap**: no 3+ consecutive sections with image+text-split layout?
...
- [ ] **Copy Self-Audit**: every visible string re-read, no grammatically-broken phrases?
- [ ] **Motion motivated**: every animation justified in one sentence?
- [ ] **Marquee max-one-per-page**: no two horizontal marquees on the same page?
- [ ] **Navigation on ONE line** at desktop, height <= 80px?
- [ ] **Section-Layout-Repetition** check: at least 4 different layout families across 8 sections?
- [ ] **Real images used** (gen-tool first, then Picsum-seed)?
- [ ] **No pills/labels overlaid on images**?
- [ ] **No scroll cues** (`Scroll`, `down scroll`, `Scroll to explore`)?
